...
export function EditableAvatar({ size = "md", editable = true }: EditableAvatarProps) {
  ...
  const AvatarMarkup = (
    <div
      className={`relative ${sizeCls} rounded-full group select-none shrink-0 ${editable ? "cursor-pointer" : ""}`}
    >
      <Avatar className="w-full h-full border border-border shadow-sm">
        {currentAvatar && (
          <AvatarImage src={currentAvatar} alt={profile?.full_name} className="object-cover" />
        )}
    ...
    <>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className="outline-none block focus-visible:ring-2 focus-visible:ring-brand-accent rounded-full"
          >
            {AvatarMarkup}
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="center" className="w-48 rounded-xl p-1.5 shadow-lg">
          <DropdownMenuItem
            ...
            className="rounded-lg py-2 flex items-center gap-2 cursor-pointer font-medium text-sm"
          >
          ...
          {profile?.avatar_url && (
            <DropdownMenuItem
              ...
              className="rounded-lg py-2 flex items-center gap-2 cursor-pointer font-medium text-sm text-red-600 hover:text-red-700 hover:bg-red-50 focus:text-red-700 focus:bg-red-50"
            >
