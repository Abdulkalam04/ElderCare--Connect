      ...
      <section className="bg-secondary/5 border border-secondary/10 rounded-3xl p-8 space-y-6">
        ...
        {existing ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            <Stat
              label="Energy"
              value={existing.energy_level ?? "—"}
              icon={<Zap className="size-4 text-amber-500" />}
            />
            <Stat
              label="Mood"
              value={existing.feeling ?? "—"}
              icon={<Smile className="size-4 text-emerald-500" />}
            />
            <Stat
              label="Meal Status"
              value={existing.meals_logged ?? (existing.ate_meals ? "Completed" : "—")}
              icon={<Utensils className="size-4 text-blue-500" />}
            />
            <Stat
              label="Medicine"
              value={
                existing.took_medicine === true
                  ? "Taken"
                  : existing.took_medicine === false
                    ? "Not taken"
                    : "—"
              }
              icon={<Pill className="size-4 text-violet-500" />}
            />
            <Stat
              label="Water Intake"
              value={
                existing.water_intake
                  ? `${existing.water_intake} Glasses`
                  : existing.drank_water
                    ? "Completed"
                    : "—"
              }
            ...
            <Stat
              label="Sleep Quality"
              value={existing.sleep_quality ?? "—"}
              icon={<Moon className="size-4 text-indigo-500" />}
            />
            <Stat
              label="Pain Status"
              value={
                existing.pain_status === false
                  ? "No Pain"
                  : existing.pain_status === true
                    ? "Has Pain"
                    : "—"
              }
    ...
    <section className="bg-secondary/5 border border-secondary/10 rounded-3xl p-6 sm:p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              { label: "Yes, taken", value: true },
              { label: "Not yet", value: false },
            ].map((option) => (
              <button
                key={option.label}
                onClick={() => set("took_medicine", option.value)}
                className={`px-3 py-1.5 rounded-xl border text-xs sm:text-sm transition-all cursor-pointer ${
                  existing?.took_medicine === option.value
