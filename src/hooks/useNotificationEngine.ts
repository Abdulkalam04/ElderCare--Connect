...
/**
 * Generates routine appointment notifications while the care-recipient app is
 * open. Missed medicines, missing wellbeing check-ins, and no-app-activity
 * alerts are intentionally handled only by the scheduled database detector so
 * they use one source of truth, respect configured thresholds, and notify all
 * linked family accounts consistently.
 */
  ...
  const notifications = appointments
    ...
    .map((appointment) => {
      ...
      return {
        ...
        message: `📅 Appointment reminder: ${appointment.title || "Appointment"} with ${appointment.doctor_name || "your doctor"
          } is scheduled for ${dateLabel} at ${timeLabel}.`,
        is_read: false,
        metadata: {
