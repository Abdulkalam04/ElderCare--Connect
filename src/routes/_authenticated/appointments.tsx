...
function AppointmentsPage() {
  ...
  const add = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["global_appointment_alarms"] });
    },
    onError: (e: Error) => {
  ...
  const remove = useMutation({
    ...
    onSuccess: (deletedId) => {
      ...
      qc.setQueryData<AppointmentRow[]>(
        ...
        (current) => current?.filter((appointment) => appointment.id !== deletedId) ?? [],
      );
      qc.invalidateQueries({ queryKey: ["appointments"] });
      qc.invalidateQueries({ queryKey: ["nextBooking"] });
      qc.invalidateQueries({ queryKey: ["global_appointment_alarms"] });
    ...
    <AppShell>
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Schedule and visits for {activeParent?.full_name ?? "—"}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {!isChildView && activeParentId && appts && appts.length > 0 && (
            <Button
              variant="outline"
              onClick={() => {
                if (
      ...
      {isChildView && (
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3 text-sm text-amber-800">
          ...
          You are viewing {activeParent?.full_name}&apos;s schedule in read-only mode. You cannot
      ...
      {!appts || appts.length === 0 ? (
        <div className="bg-card border border-border rounded-3xl p-16 text-center text-muted-foreground">
          ...
          {!isChildView && (
            <p className="text-sm mt-1">Click the button above to add a new appointment.</p>
          )}
        </div>
      ) : (
        <>
                          ...
                          {!isChildView && (
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              <button
                                ...
                                disabled={remove.isPending && remove.variables === appt.id}
                                className="p-2 text-stone-500 hover:text-destructive transition-colors cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
                  ...
                  <div className="bg-card border border-border rounded-3xl overflow-hidden divide-y divide-border opacity-70">
                    {pastAppts.map((appt) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="font-semibold text-base text-muted-foreground truncate">
                              {appt.title}
                            </p>
                            {appt.reminder_enabled && (
                        ...
                        {!isChildView && (
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <button
                              onClick={() => handleDeleteClick(appt)}
                              disabled={remove.isPending && remove.variables === appt.id}
                              className="p-2 text-stone-400 hover:text-destructive transition-colors cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
                              title="Delete appointment"
                              aria-label={`Delete ${appt.title}`}
                            >
                              <Trash2 className="size-4" />
                        ...
                        >
                          ...
                          {!isChildView && (
                            <div className="flex flex-col gap-1 shrink-0">
                              <button
                                onClick={() => handleEditClick(appt)}
                                className="p-1.5 text-stone-400 hover:text-stone-800 transition-colors cursor-pointer"
                                title="Edit appointment"
                              >
                              ...
                              <button
                                onClick={() => handleDeleteClick(appt)}
                                disabled={remove.isPending && remove.variables === appt.id}
                                className="p-1.5 text-stone-400 hover:text-destructive transition-colors cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
                                title="Delete appointment"
                                aria-label={`Delete ${appt.title}`}
                              >
            ...
            <div className="flex items-center justify-between p-3 bg-stone-50 rounded-2xl border border-stone-100">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-semibold">Enable Appointment Alarm</span>
                  <span className="text-[10px] text-muted-foreground">
                    Alarm and notification at the selected date and time
                  </span>
                </div>
              </div>
              <input
                ...
                onChange={(e) => {
                  void handleReminderEnabledChange(e.target.checked);
                }}
