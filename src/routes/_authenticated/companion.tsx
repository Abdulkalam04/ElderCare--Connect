...
function CompanionPage() {
  ...
  async function addFiles(files: FileList | null, kindHint?: "image" | "file") {
    ...
    for (const f of Array.from(files)) {
      ...
      if (!isImage && !isPdf) {
        toast.error(`"${f.name}" — only images and PDF are supported.`);
        continue;
      }
      try {
    ...
    if (next.length) {
      ...
      toast.success(`📎 ${next.length} attachment${next.length > 1 ? "s" : ""} added`);
    }
  }
  ...
  type ApiAttachment = { kind: "image" | "file"; name?: string; mime: string; dataUrl: string };
  type HistoryMsg = { role: "user" | "assistant"; content: string; attachments?: ApiAttachment[] };

  const sendWithRetry = useCallback(
    async (
      text: string,
      historyOverride?: HistoryMsg[],
      ...
      const history: HistoryMsg[] =
        historyOverride ?? [
          ...
          { role: "user" as const, content: text, attachments: currentAttachments },
        ];
  ...
  async function send() {
    ...
    const attachmentsLabel = hasAttachments
      ? attachments
          .map((a) => (a.kind === "image" ? `🖼️ ${a.name}` : `📎 ${a.name}`))
          .join("  ")
      : "";
    const storedText = [text, attachmentsLabel].filter(Boolean).join("\n");
    ...
    if (isRateLimited) {
      ...
      if (hasAttachments) {
        toast.warning("Attachments aren't queued — try again after the cooldown.");
      }
      const queueItem: QueuedMessage = { id: crypto.randomUUID(), text: storedText || text, addedAt: Date.now() };
      setQueue((prev) => [...prev, queueItem]);
      await supabase.from("ai_chat_messages").insert({ parent_id: activeParentId, role: "user", content: storedText || text });
      await qc.invalidateQueries({ queryKey: ["aiChat"] });
    ...
    setSending(true);
    try {
      await supabase.from("ai_chat_messages").insert({ parent_id: activeParentId, role: "user", content: storedText || "(no text)" });
      await qc.invalidateQueries({ queryKey: ["aiChat"] });
      const history: HistoryMsg[] = [
      ...
      await sendWithRetry(text, history, currentAttachments);
    } catch (e) {
              ...
              {attachments.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {attachments.map((a) => (
                    <div
                      key={a.id}
                      className="relative group flex items-center gap-2 pl-1 pr-7 py-1 rounded-lg bg-blue-50 border border-blue-200 text-xs text-blue-900 max-w-[200px]"
                    >
                      {a.kind === "image" && a.previewUrl ? (
                        <img src={a.previewUrl} alt={a.name} className="size-8 rounded object-cover" />
                      ) : (
                        <FileText className="size-5 text-blue-600 ml-1" />
                      )}
                      <span className="truncate">{a.name}</span>
                      <button
                        type="button"
                        onClick={() => removeAttachment(a.id)}
                        className="absolute top-1 right-1 rounded-full p-0.5 hover:bg-blue-200"
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex gap-1">
                  <Button
                    ...
                    disabled={!activeParentId || !isViewingToday || attachments.length >= MAX_ATTACHMENTS}
                    title="Add image"
                  >
