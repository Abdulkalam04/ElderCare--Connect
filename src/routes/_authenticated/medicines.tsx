...
function MedicinesPage() {
  ...
  const add = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["global_meds"] });
    },
    onError: (e: Error) => {
  ...
  const remove = useMutation({
    ...
    onSuccess: (deletedId) => {
      ...
      qc.invalidateQueries({ queryKey: ["global_taken_meds"] });
    },
  ...
  const clearAll = useMutation({
    mutationFn: async () => {
      if (isChildView) throw new Error("You do not have permission to perform this action.");
      const { error } = await supabase.from("medicines").delete().eq("parent_id", activeParentId!);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <AppShell>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <TimeInput
                          ...
                          onChange={(v) => setForm({ ...form, schedule_time: v })}
                          placeholder="HH:MM"
                        />
                      </div>
                    </div>
      ...
      {isChildView && (
        <div className="mb-6 bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3 text-sm text-blue-700">
          ...
          You are viewing {activeParent?.full_name}&apos;s medications in read-only mode. Use the
          buttons above to send reminders or call.
        </div>
      )}
            ...
            {meds.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {m.duration && (
                        <Badge
                          variant="outline"
                          className="text-[10px] font-semibold px-2 py-0.5 rounded-full border-primary/30 text-primary bg-primary/5"
                        >
                          {m.duration}
                        </Badge>
                      )}
                  ...
                  <span
                    className={`text-xs font-mono px-2 py-1 rounded capitalize ${periodColors[m.period] ?? "bg-stone-100 text-stone-700"}`}
                  >
                    {m.period}
                  </span>
                  ...
                  {!isChildView && (
                    <>
                      ...
                      <button
                        onClick={() => {
                          if (
                            confirm(`Delete medication "${m.name}"? This action cannot be undone.`)
                          ) {
                            remove.mutate(m.id);
                          }
                        }}
                        disabled={remove.isPending && remove.variables === m.id}
                        className="text-muted-foreground hover:text-destructive p-2 cursor-pointer transition-colors disabled:cursor-not-allowed disabled:opacity-50"
